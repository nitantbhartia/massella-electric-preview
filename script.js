(function () {
  ...
  function applyLang(lang) {
    ...
    try { localStorage.setItem("massella-lang", lang); } catch (e) {}
  }
  document.querySelectorAll(".lang button").forEach(function (btn) {
    btn.addEventListener("click", function () { applyLang(btn.getAttribute("data-set")); });
  });
  var saved = null;
  try { saved = localStorage.getItem("massella-lang"); } catch (e) {}
  ...
  if (form) {
    form.addEventListener("submit", function (e) {
      ...
      var body = "Massella Electric job\nName: " + name + "\nAddress: " + address + "\nNeed: " + need;
